...
def test_list_images_for_org():
    ...
    user = User.objects.create_user(
        email="imgtest@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    img1 = Image.objects.create(
        file=create_test_image_file(name="test1.png"),
        organization=org,
        creator=user,
        description="desc1",
        title="img1",
    ...
    img2 = Image.objects.create(
        file=create_test_image_file(name="test2.png"),
        organization=org,
        creator=user,
        description="desc2",
        title="img2",
...
def test_upload_image():
    ...
    user = User.objects.create_user(
        email="upload@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    client = Client()
...
def test_bulk_upload_images():
    ...
    user = User.objects.create_user(
        email="bulk@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    client = Client()
    ...
    response = client.post(
        f"/api/v1/orgs/{org.slug}/bulk-upload/",
        files_dict,
        HTTP_AUTHORIZATION=f"Bearer {access}",
    )
    assert response.status_code == 200, response.content
...
def test_delete_image():
    ...
    user = User.objects.create_user(
        email="delete@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    ...
    response = client.delete(
        f"/api/v1/orgs/{org.slug}/images/{img.id}/",
        HTTP_AUTHORIZATION=f"Bearer {access}",
    )
...
def test_bulk_delete_images():
    ...
    user = User.objects.create_user(
        email="bulkdel@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    imgs = [
        Image.objects.create(
            file=create_test_image_file(name=f"bd{i}.png"),
            organization=org,
            creator=user,
        )
        for i in range(2)
    ...
    response = client.post(
        f"/api/v1/orgs/{org.slug}/bulk-delete/",
        {"ids": ids},
        content_type="application/json",
        HTTP_AUTHORIZATION=f"Bearer {access}",
    )
...
def test_bulk_delete_images_reports_missing_ids():
    org = Organization.objects.create(
        name="BulkDelPartialOrg", slug="bulkdelpartialorg"
    )
    user = User.objects.create_user(
        email="bulkdelpartial@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    img = Image.objects.create(
        file=create_test_image_file(name="bdp.png"), organization=org, creator=user
    )
...
def test_bulk_delete_images_reports_no_deletions():
    ...
    user = User.objects.create_user(
        email="bulkdelnone@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    missing_id = 999999
    client = Client()
...
def test_attach_and_detach_image():
    ...
    user = User.objects.create_user(
        email="attach@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    img = Image.objects.create(
        file=create_test_image_file(name="att.png"), organization=org, creator=user
    )
    ...
    attach_url = f"/api/v1/orgs/{org.slug}/images/{app_label}/{model}/{object_id}/"
    response = client.post(
        attach_url,
    ...
    detach_url = (
        f"/api/v1/orgs/{org.slug}/images/{app_label}/{model}/{object_id}/{img.id}/"
    )
...
def test_list_images_for_object():
    ...
    user = User.objects.create_user(
        email="objuser@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    img = Image.objects.create(
        file=create_test_image_file(name="objimg.png"), organization=org, creator=user
    )
    ...
    PolymorphicImageRelation.objects.create(
        image=img, content_type=ct, object_id=object_id
    )

    client = Client()
    access = get_access_token("objuser@example.com", "pw")
    url = f"/api/v1/orgs/{org.slug}/images/{app_label}/{model}/{object_id}/"
    response = client.get(url, HTTP_AUTHORIZATION=f"Bearer {access}")
    assert response.status_code == 200
    data = response.json()
    assert "items" in data
    ...
    response = client.get(
        f"/api/v1/orgs/{org.slug}/images/{app_label}/{model}/999999/",
        HTTP_AUTHORIZATION=f"Bearer {access}",
    )
    assert response.status_code == 404
...
def test_unauthorized_access():
    ...
    user = User.objects.create_user(
        email="noauth@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    img = Image.objects.create(
        file=create_test_image_file(name="noauth.png"), organization=org, creator=user
    )
    ...
    response = client.post(
        f"/api/v1/orgs/{org.slug}/images/",
        files={"file": create_test_image_file(name="fail.png")},
    )
    assert response.status_code in (401, 403)
    response = client.delete(f"/api/v1/orgs/{org.slug}/images/{img.id}/")
    assert response.status_code in (401, 403)
...
def test_upload_invalid_file():
    ...
    user = User.objects.create_user(
        email="invalid@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    client = Client()
...
def test_attach_images_to_object():
    ...
    Membership.objects.create(user=user, organization=org, role="owner")
    img = Image.objects.create(
        file=create_test_image_file(name="attachimg.png"),
        organization=org,
        creator=user,
    )
    app_label = "organizations"
    ...
    assert response.status_code == 200
    data = response.json()
    assert any(r["image"]["id"] == img.id for r in data) or any(
...
def test_remove_image_from_object():
    ...
    user = User.objects.create_user(
        email="remove@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    img = Image.objects.create(
        file=create_test_image_file(name="removeimg.png"),
        organization=org,
        creator=user,
    )
    app_label = "organizations"
    ...
    assert access, "Failed to get access token. Check credentials and token endpoint."
    url = f"/api/v1/orgs/{org.slug}/images/{app_label}/{model}/{object_id}/{img.id}/"
