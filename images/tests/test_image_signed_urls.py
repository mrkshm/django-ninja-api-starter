...
def test_org_member_can_get_signed_image_urls(monkeypatch):
    ...
    monkeypatch.setattr("images.services.generate_private_presigned_storage_url", fake_presign)

    User = get_user_model()
    user = User.objects.create_user(email="signed@example.com", password="pw", email_verified=True)
    ...
    response = Client().get(
        f"/api/v1/images/orgs/{org.slug}/images/{image.id}/urls",
    ...
    assert ("images/example_thumb.webp", {"expires_in": 900, "content_type": "image/webp", "cache_control": "private, max-age=900"}) in calls


@pytest.mark.django_db
def test_non_member_cannot_get_signed_image_urls(monkeypatch):
    monkeypatch.setattr("images.services.generate_private_presigned_storage_url", lambda *args, **kwargs: "unused")

    User = get_user_model()
    owner = User.objects.create_user(email="owner@example.com", password="pw", email_verified=True)
    outsider = User.objects.create_user(email="outsider@example.com", password="pw", email_verified=True)
    ...
    image = Image.objects.create(file="images/private.jpg", organization=org, creator=owner)
...
def test_org_member_can_create_and_revoke_share_link(monkeypatch):
    monkeypatch.setattr("images.services.generate_private_presigned_storage_url", lambda key, **kwargs: f"https://r2.example/{key}")

    User = get_user_model()
    user = User.objects.create_user(email="share@example.com", password="pw", email_verified=True)
    org = Organization.objects.create(name="Share URL Org", slug="share-url-org")
    Membership.objects.create(user=user, organization=org, role="owner")
    image = Image.objects.create(file="images/share.jpg", organization=org, creator=user)

    access = get_access_token("share@example.com", "pw")
    client = Client()
    create_response = client.post(
        f"/api/v1/images/orgs/{org.slug}/images/{image.id}/shares",
    ...
    shared_response = client.get(f"/api/v1/images/shared/images/{share_data['token']}/urls")
    assert shared_response.status_code == 200, shared_response.content
    assert shared_response.json()["urls"]["original"] == "https://r2.example/images/share.jpg"

    revoke_response = client.delete(
        f"/api/v1/images/orgs/{org.slug}/images/{image.id}/shares/{share_data['id']}",
        HTTP_AUTHORIZATION=f"Bearer {access}",
    )
...
def test_expired_share_link_cannot_get_signed_urls(monkeypatch):
    ...
    response = Client().get(f"/api/v1/images/shared/images/{share_link.token}/urls")
