...
def test_attach_defaults_primary_and_order():
    ...
    user = User.objects.create_user(
        email="a@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")

    img1 = Image.objects.create(
        file=create_test_image_file(name="a.png"), organization=org, creator=user
    )
    img2 = Image.objects.create(
        file=create_test_image_file(name="b.png"), organization=org, creator=user
    )
    ...
    # attach both
    url = f"/api/v1/orgs/{org.slug}/images/{app_label}/{model}/{obj_id}/"
    resp = client.post(
        url,
        data={"image_ids": [img1.id]},
        content_type="application/json",
...
def test_reorder_sets_primary_and_orders():
    ...
    user = User.objects.create_user(
        email="b@example.com", password="pw", email_verified=True
    )
    Membership.objects.create(user=user, organization=org, role="owner")
    ...
    reorder_url = f"/api/v1/orgs/{org.slug}/images/{app_label}/{model}/{obj_id}/reorder"
    resp = client.post(
        reorder_url,
        data={"image_ids": new_order},
        content_type="application/json",
        HTTP_AUTHORIZATION=f"Bearer {token}",
    )
    assert resp.status_code == 200
