...
def test_avatar_upload_too_large(monkeypatch, api_client):
    ...
    response = api_client.post("/token/pair", json={"email": "bigavatar@example.com", "password": "pw"})
...
def test_avatar_upload_invalid_content_type(api_client):
    ...
    response = api_client.post("/token/pair", json={"email": "badtype@example.com", "password": "pw"})
    access = response.json()["access"]
    headers = {"Authorization": f"Bearer {access}"}
...
def test_avatar_upload_corrupt_image(api_client):
    ...
    response = api_client.post("/token/pair", json={"email": "corruptimg@example.com", "password": "pw"})
    access = response.json()["access"]
    headers = {"Authorization": f"Bearer {access}"}
...
@pytest.mark.django_db
def test_avatar_delete_authenticated(monkeypatch, api_client):
    user = create_test_user(email="delavatar@example.com", password="pw")
    user.avatar_path = "avatars/delme.webp"
    user.save()
    response = api_client.post("/token/pair", json={"email": "delavatar@example.com", "password": "pw"})
    access = response.json()["access"]
    headers = {"Authorization": f"Bearer {access}"}
    called = {}
...
@pytest.mark.django_db
def test_avatar_delete_when_no_avatar(monkeypatch, api_client):
    user = create_test_user(email="noavatar@example.com", password="pw")
    user.avatar_path = None
    user.save()
    response = api_client.post("/token/pair", json={"email": "noavatar@example.com", "password": "pw"})
    access = response.json()["access"]
    headers = {"Authorization": f"Bearer {access}"}
    called = {}
    def fake_delete_existing_avatar(u):
...
def test_avatar_upload_valid(monkeypatch, api_client):
    ...
    response = api_client.post("/token/pair", json={"email": "validavatar@example.com", "password": "pw"})
    access = response.json()["access"]
    headers = {"Authorization": f"Bearer {access}"}
