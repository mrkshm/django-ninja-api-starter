...
def test_request_email_change_success(settings, api_client):
    ...
    resp = api_client.post('/token/pair', json={'email': 'old@example.com', 'password': 'pw'})
    access = resp.json()['access']
    headers = {'Authorization': f'Bearer {access}'}
    resp = api_client.patch('/auth/email', json={'email': 'new@example.com'}, headers=headers)
...
def test_verify_email_change_success(settings, api_client):
    ...
    PendingEmailChange.objects.create(user=user, new_email='new2@example.com', token=hash_token(token), expires_at=expires)
    resp = api_client.get(f'/auth/email/verify?token={token}')
    assert resp.status_code == 200
...
def test_verify_email_change_expired_token(settings, api_client):
    ...
    expires = timezone.now() - timezone.timedelta(hours=1)
    PendingEmailChange.objects.create(user=user, new_email='expired2@example.com', token=hash_token(token), expires_at=expires)
    resp = api_client.get(f'/auth/email/verify?token={token}')
...
def test_request_email_change_invalid_format(settings, api_client):
    ...
    resp = api_client.post('/token/pair', json={'email': 'invalid1@example.com', 'password': 'pw'})
    access = resp.json()['access']
    headers = {'Authorization': f'Bearer {access}'}
    resp = api_client.patch('/auth/email', json={'email': 'not-an-email'}, headers=headers)
...
def test_request_email_change_email_taken(settings, api_client):
    ...
    resp = api_client.post('/token/pair', json={'email': 'user2@example.com', 'password': 'pw'})
    access = resp.json()['access']
    headers = {'Authorization': f'Bearer {access}'}
    resp = api_client.patch('/auth/email', json={'email': 'taken@example.com'}, headers=headers)
    assert resp.status_code == 400
...
def test_multiple_pending_changes(settings, api_client):
    ...
    resp = api_client.post('/token/pair', json={'email': 'multi@example.com', 'password': 'pw'})
    access = resp.json()['access']
    headers = {'Authorization': f'Bearer {access}'}
    # First request
    resp1 = api_client.patch('/auth/email', json={'email': 'first@example.com'}, headers=headers)
    assert resp1.status_code == 200
    # Second request before verifying
    resp2 = api_client.patch('/auth/email', json={'email': 'second@example.com'}, headers=headers)
...
def test_email_change_after_verification(settings, api_client):
    ...
    expires = timezone.now() + timezone.timedelta(hours=1)
    PendingEmailChange.objects.create(user=user, new_email='afterverify2@example.com', token=hash_token(token), expires_at=expires)
    resp = api_client.get(f'/auth/email/verify?token={token}')
    assert resp.status_code == 200
    user.refresh_from_db()
...
def test_case_insensitive_email_uniqueness(settings, api_client):
    ...
    resp = api_client.post('/token/pair', json={'email': 'other@example.com', 'password': 'pw'})
    access = resp.json()['access']
    headers = {'Authorization': f'Bearer {access}'}
    # Try to change to same email with different case
    resp = api_client.patch('/auth/email', json={'email': 'CaseUser@Example.com'}, headers=headers)
    assert resp.status_code == 400
    assert 'Email already taken' in resp.json()['detail']
...
def test_reusing_token_after_success(settings, api_client):
    ...
    token = 'reusetoken123'
    expires = timezone.now() + timezone.timedelta(hours=1)
    PendingEmailChange.objects.create(user=user, new_email='reuse2@example.com', token=hash_token(token), expires_at=expires)
    # First use (should succeed)
    resp1 = api_client.get(f'/auth/email/verify?token={token}')
    assert resp1.status_code == 200
    user.refresh_from_db()
    assert user.email == 'reuse2@example.com'
    # Second use (should fail)
    resp2 = api_client.get(f'/auth/email/verify?token={token}')
...
def test_verify_email_change_email_taken(api_client):
    ...
    token = 'tokentaken123'
    expires = timezone.now() + timezone.timedelta(hours=1)
    PendingEmailChange.objects.create(user=user2, new_email='taken@example.com', token=hash_token(token), expires_at=expires)
